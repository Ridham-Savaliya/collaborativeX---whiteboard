...
interface SidebarProps { 
  ...
  addStickyNote: (note: { id: string; position: { x: number; y: number }; size: { width: number; height: number }; color: string; text: string }) => void;
}
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const tools = [
    ...
    { key: 'stickyNote', label: 'Sticky Note', icon: <StickyNote size={16} />, tooltip: 'Add a sticky note (draggable)' },
  ...
  // Handle canvas click for sticky note placement
  useEffect(() => {
  ...
  if (isCollapsed) {
    return (
      <div className="fixed top-0 left-0 h-full z-10 flex transition-all duration-500 ease-in-out">
        <div className="h-full bg-gray-800/80 backdrop-blur-xl text-white px-2 py-5 flex flex-col items-center justify-start rounded-r-xl shadow-2xl border-r border-purple-500/20">
          <button
        ...
        <div 
          className={`absolute left-14 top-4 bg-black/80 backdrop-blur-sm text-white text-xs font-medium px-2 py-1 rounded-md pointer-events-none transition-opacity duration-300 ${currentTool ? 'opacity-100' : 'opacity-0'}`}
        >
  ...
  return (
    <div className="fixed top-0 left-0 h-full z-10 transition-all duration-500 ease-in-out">
      <div className="h-full bg-gray-800/90 backdrop-blur-xl text-white w-64 sm:w-72 py-4 px-4 flex flex-col rounded-r-xl shadow-2xl border-r border-purple-500/20 overflow-y-scroll scrollbar-custom">
